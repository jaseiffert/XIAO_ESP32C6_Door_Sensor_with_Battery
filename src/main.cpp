...
# ################################################################################################ #
# File: main.cpp                                                                                   #
# Project: XIAO ESP32C6 Battery Door Sensor                                                        #
# Created Date: Wednesday, October 15th 2025, 13:27:20                                             #
# Author: Jeffery Seiffert                                                                         #
# -----                                                                                            #
# Last Modified: Wed Oct 15 2025                                                                   #
...
void setup() {
    ...
    */
    ...
    switchState = digitalRead(SENSOR_SWITCH);

    if (switchState) {
        if (esp_deep_sleep_enable_gpio_wakeup(BIT(SENSOR_SWITCH), ESP_GPIO_WAKEUP_GPIO_LOW) == ESP_OK) {
            debugpln("Starting Sleep GPIO");
        }
        else {
            debugpln("Error Starting Sleep GPIO");
        }
        digitalWrite(LED_GREEN, HIGH);
    }
    else {
        if (esp_deep_sleep_enable_gpio_wakeup(BIT(SENSOR_SWITCH), ESP_GPIO_WAKEUP_GPIO_HIGH) == ESP_OK) {
            debugpln("Starting Sleep GPIO");
        }
        else {
            debugpln("Error Starting Sleep GPIO");
...
void loop() {
    ...
    if (switchState == HIGH && last_switchState == LOW) {
        switch_time = millis();
        String response = "";
        if (switch_time - last_switch_time > 250) {
            debugpln("Switch Closed.");
            sendMQTT();
            digitalWrite(LED_GREEN, HIGH);
            last_switch_time = switch_time;
        }
        // switch state changed reset sleep timer
    ...
    if (switchState == LOW && last_switchState == HIGH) {
        switch_time = millis();
        String response = "";
        if (switch_time - last_switch_time > 250) {
            debugpln("Switch Open.");
            sendMQTT();
            digitalWrite(LED_RED, HIGH);
            last_switch_time = switch_time;
            last_switchState = 0;
...
String checkSwitchState() {
    String data = "";
    data += "\"switchstate\":\"" + String(switchState ? "Closed" : "Open") + "\"";
    debugp("Data String: ");
    debugpln((data));
